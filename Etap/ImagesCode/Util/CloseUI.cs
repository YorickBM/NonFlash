...
namespace Util
{
    class CloseUI
    {
        ...
        internal Font title;
        internal HoverButton CloseB;
        internal int headerHeight = 31;

        public CloseUI(ContentManager content, Vector2i position, Vector2i size, string title, int offsetX = 0, int offsetY = 0)
        {
        ...
        public virtual void Draw(SpriteBatch spriteBatch, float depth)
        {
            if (allowRender)
            {
                bottomBackground.Draw(spriteBatch, (position + offset + new Vector2i(bottomLeftEdge.dimensions.X, headerHeight)), depth - 0.03f);

                leftCornerTop.Draw(spriteBatch, (position + offset), 0.9f);
                rightCornerTop.Draw(spriteBatch, (position + offset + new Vector2i(size.X - rightCornerTop.dimensions.X, 0)), depth - 0.02f, SpriteEffects.FlipHorizontally);
                edgeTop.Draw(spriteBatch, (position + offset + new Vector2i(leftCornerTop.dimensions.X - 1, 0)), depth - 0.02f);

                topLeftEdge.Draw(spriteBatch, (position + offset + new Vector2i(0, leftCornerTop.dimensions.Y)), depth - 0.02f);
                topRightEdge.Draw(spriteBatch, (position + offset + new Vector2i(size.X - rightCornerTop.dimensions.X + 1, leftCornerTop.dimensions.Y)), depth - 0.02f, SpriteEffects.FlipHorizontally);

                leftCornerBottom.Draw(spriteBatch, (position + offset + new Vector2i(0, size.Y - leftCornerBottom.dimensions.Y)), depth - 0.02f);
                rightCornerBottom.Draw(spriteBatch, (position + offset + new Vector2i(size.X - rightCornerTop.dimensions.X, size.Y - leftCornerBottom.dimensions.Y)), depth - 0.02f, SpriteEffects.FlipHorizontally);
                edgeBottom.Draw(spriteBatch, (position + offset + new Vector2i(leftCornerBottom.dimensions.X - 1, size.Y - edgeBottom.dimensions.Y)), depth - 0.02f);
                bottomLeftEdge.Draw(spriteBatch, (position + offset + new Vector2i(0, headerHeight)), depth - 0.02f);
                bottomRightEdge.Draw(spriteBatch, (position + offset + new Vector2i(size.X - bottomRightEdge.dimensions.X, headerHeight)), depth - 0.02f, SpriteEffects.FlipHorizontally);

                CloseB.Enable();
                CloseB.Draw(spriteBatch, (position + offset + new Vector2i(size.X - leftCornerTop.dimensions.X - (CloseB.dimensions.X / 2) + 8, 2 + (int)(headerHeight / 2 - CloseB.dimensions.Y / 2))), depth - 0.01f);
                title.Draw(spriteBatch, position + offset + new Vector2((int)(size.X / 2 - title.measureString().X / 2), 2 + (int)(headerHeight / 2 - title.measureString().Y / 2)), depth);
            } else
            {
                CloseB.Disable();
            }
