...
namespace Etap.Communication.ConnectionManager
{
	public class ConnectionInformation : IDisposable
	{
		...
		public ConnectionInformation(int connectionID, Socket dataStream, ISocketManager manager, IDataParser parser, string ip)
		{
			this.parser = parser;
		...
		private void incomingDataPacket(IAsyncResult iAr)
		{
			...
			if (bytesReceived == 0)
			{
                Logger.Error("Disconnecting Connection", "bytesReceived == 0");
                disconnect();
				return;
			}
			...
			{
                Logger.Error("Disconnecting Connection", e);
                disconnect();
			}
			finally
			{
		...
		public void SendData(byte[] packet)
		{
			try
			{
				...
				Logger.Debug(string.Format("Data too server => [{0}]", packetData));
