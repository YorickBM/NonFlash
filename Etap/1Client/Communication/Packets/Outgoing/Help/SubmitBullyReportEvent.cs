...
namespace Retro.Communication.Packets.Incoming.Help
{
    class SubmitBullyReportEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            GameClient Client = RetroEnvironment.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(UserId));
            if (Client == null)
            {
            ...
            if (Session.GetHabbo().LastAdvertiseReport > RetroEnvironment.GetUnixTimestamp())
            {
                Session.SendNotification("Rapporten kunnen alleen om de 5 minuten worden verzonden!");
                return;
            }

            if (Client.GetHabbo().GetPermissions().HasRight("mod_tool"))//Reporting staff, nope!
            {
                Session.SendNotification("Sorry, je kunt teamleden niet informeren via deze tool.");
                return;
            }
            ...
            if (Session.GetHabbo().Rank <= 1)
                Session.GetHabbo().LastAdvertiseReport = RetroEnvironment.GetUnixTimestamp() + 300;
            else
                Session.GetHabbo().LastAdvertiseReport = RetroEnvironment.GetUnixTimestamp();

            Client.GetHabbo().AdvertisingReported = true;
            Session.SendMessage(new SubmitBullyReportComposer(0));
            //RetroEnvironment.GetGame().GetClientManager().ModAlert("New advertising report! " + Client.GetHabbo().Username + " has been reported for advertising by " + Session.GetHabbo().Username +".");
            RetroEnvironment.GetGame().GetClientManager().DoAdvertisingReport(Session, Client);     
            return;
        }
    }
}