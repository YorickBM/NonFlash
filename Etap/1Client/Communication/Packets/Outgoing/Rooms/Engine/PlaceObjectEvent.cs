...
using Retro.Hotel.Items.Data.Toner;
...
namespace Retro.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Room.ForSale)
            {
                Session.SendWhisper("You can not edit the room while it is for sale");
                Session.SendWhisper("To cancel the sale, type ': unload' (without '')");
                return;
            }

            if (Room.GetRoomItemHandler().GetWallAndFloor.Count() > Convert.ToInt32(RetroEnvironment.GetGame().GetSettingsManager().TryGetValue("room.item.placement_limit")))
            {
                Session.SendNotification("You can not have more than " + Convert.ToInt32(RetroEnvironment.GetGame().GetSettingsManager().TryGetValue("room.item.placement_limit")) + " furniture in your room");
                return;
            }
            ...
            {
                Session.SendNotification("You can not put changed coins in this room!");
                return;
            }
                ...
                case InteractionType.MOODLIGHT:
                    {
                        ...
                        {
                            Session.SendNotification("You can have only one (1) Moodlight!");
                            return;
                        }
                        break;
                    }
                case InteractionType.TONER:
                    {
                        ...
                        {
                            Session.SendNotification("You can have only one (1) Background room painted!");
                            return;
                        }
                        break;
                    }
                case InteractionType.HOPPER:
                    {
                        ...
                        {
                            Session.SendNotification("You can only have one (1) Hopper in this room!");
                            return;
                        }
                        break;
                    }
            ...
            if (!Item.IsWallItem)
            {
                ...
                if (Room.GetRoomItemHandler().SetFloorItem(Session, RoomItem, X, Y, Rotation, true, false, true))
                {
                    ...
                    if (Session.GetHabbo().Id == Room.OwnerId)
                        RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_RoomDecoFurniCount", 1, false);
