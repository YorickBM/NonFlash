...
namespace Retro.Communication.Packets.Incoming.Rooms.FloorPlan
{
	class SaveFloorPlanModelEvent : IPacketEvent
	{
		public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
		{
			...
			{
				Session.SendMessage(new RoomNotificationComposer("floorplan_editor.error", "errors", "Oops, it looks like you typed an invalid floor!"));
				return;
			}
			...
			if (!isValid)
			{
				Session.SendMessage(new RoomNotificationComposer("floorplan_editor.error", "errors", "Oops, it looks like you came in the room with the floor invalid!"));
				return;
			}
            ...
            RetroEnvironment.GetGame().GetRoomManager().ReloadModel(ModelName);
            RetroEnvironment.GetGame().GetRoomManager().UnloadRoom(Room);
