...
namespace Retro.Communication.Packets.Incoming.Rooms.Furni
{
	class OpenGiftEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            UserCache Purchaser = RetroEnvironment.GetGame().GetCacheManager().GenerateUser(PurchaserId);
            if (Purchaser == null)
            {
                Session.SendNotification("Ops! aparentemente há um erro com este presente.\nVamos livrar-se deste presente.");
                Room.GetRoomItemHandler().RemoveFurniture(null, Present.Id, false);
            ...
            if (!RetroEnvironment.GetGame().GetItemManager().GetItem(Convert.ToInt32(Data["base_id"]), out BaseItem))
            {
