...
namespace Retro.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogAsGiftEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (RetroEnvironment.GetGame().GetSettingsManager().TryGetValue("room.item.gifts.enabled") != "1")
            {
                Session.SendNotification("Hotelmanagers hebben cadeaus geven uitgeschakeld!");
                return;
            }
            ...
            if (!RetroEnvironment.GetGame().GetItemManager().GetGift(SpriteId, out PresentData) || PresentData.InteractionType != InteractionType.GIFT)
                return;
            ...
            Habbo Habbo = RetroEnvironment.GetHabboByUsername(GiftUser);
            if (Habbo == null)
            {
            ...
            if (!Habbo.AllowGifts)
            {
                Session.SendNotification("Oops! Deze gebruiker accepteert geen geschenken!");
                return;
            }

            if ((DateTime.Now - Session.GetHabbo().LastGiftPurchaseTime).TotalSeconds <= 15.0)
            {
                Session.SendNotification("Je koopt heel snel cadeaus! Wacht 15 seconden!");
            ...
            using (IQueryAdapter dbClient = RetroEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                switch (Item.Data.InteractionType)
                {
                    ...
                    case InteractionType.LANDSCAPE:
                        ...
                        try
                        {
                            if (string.IsNullOrEmpty(Data))
                                ...
                                Number = Double.Parse(Data, RetroEnvironment.CultureInfo);
                        }
                        catch
                        {
                    ...
                    case InteractionType.BADGE_DISPLAY:
                        ...
                        {
                            Session.SendMessage(new BroadcastMessageAlertComposer("Hé, het ziet ernaar uit dat deze badge niet van jou is!"));
                            return;
                        }
            ...
            if (GiveItem != null)
            {
                GameClient Receiver = RetroEnvironment.GetGame().GetClientManager().GetClientByUserID(Habbo.Id);
                if (Receiver != null)
                {
                ...
                {
                    RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_GiftGiver", 1);
                    if (Receiver != null)
                        RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Receiver, "ACH_GiftReceiver", 1);
                    RetroEnvironment.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.GIFT_OTHERS);
                }
            }
