...
using Retro.Hotel.Rooms.AI.Responses;
...
namespace Retro.Communication.Packets.Incoming.Catalog
{
    class CheckGnomeNameEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (!RetroEnvironment.IsValidAlphaNumeric(PetName))
            {
            ...
            if (Pet == null)
            {
                Session.SendNotification("Oops! Er is een fout opgetreden, meld dit alstublieft!");
                return;
            }
            ...
            if (RetroEnvironment.GetGame().GetItemManager().GetItem(320, out PetFood))
            {
