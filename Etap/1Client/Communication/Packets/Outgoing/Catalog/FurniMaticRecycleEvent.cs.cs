    ...
    class FurniMaticRecycleEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            for (int i = 0; i < itemsCount; i++)
            {
                ...
                using (var dbClient = RetroEnvironment.GetDatabaseManager().GetQueryReactor()) dbClient.runFastQuery("DELETE FROM `items` WHERE `id` = '" + itemId + "' AND `user_id` = '" + Session.GetHabbo().Id + "' LIMIT 1");
                Session.GetHabbo().GetInventoryComponent().RemoveItem(itemId);
            }

            var reward = RetroEnvironment.GetGame().GetFurniMaticRewardsMnager().GetRandomReward();
            if (reward == null) return;
            int rewardId;
            var furniMaticBoxId = 4692;
            ItemData data = null;
            RetroEnvironment.GetGame().GetItemManager().GetItem(furniMaticBoxId, out data);
            ...
            RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_FurnimaticQuest", 1);
        }
    }
}