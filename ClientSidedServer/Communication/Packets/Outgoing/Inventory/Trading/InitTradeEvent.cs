...
using Retro.Communication.Packets.Outgoing.Inventory.Trading;
...
namespace Retro.Communication.Packets.Incoming.Inventory.Trading
{
    class InitTradeEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                if (Session.GetHabbo().TradingLockExpiry > RetroEnvironment.GetUnixTimestamp())
                {
                    Session.SendNotification("Uw account is momenteel geblokkeerd voor trading!");
                    return;
                }
                else
                {
                    Session.GetHabbo().TradingLockExpiry = 0;
                    Session.SendNotification("Uw trading is weer actief, maak de fout niet opnieuw!");
            ...
            {
                Session.SendNotification("Oeps, deze gebruiker heeft blijkbaar de trading uitgeschakeld!");
                return;
            }
