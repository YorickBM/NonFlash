...
namespace Retro.Communication.Packets.Incoming.Rooms.AI.Pets
{
	class PickUpPetEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendWhisper("You can only collect your own pets, so take a pet you must have the correct rights.");
                return;
            }
            ...
            {
                GameClient Target = RetroEnvironment.GetGame().GetClientManager().GetClientByUserID(pet.OwnerId);
                if (Target != null)
                {
