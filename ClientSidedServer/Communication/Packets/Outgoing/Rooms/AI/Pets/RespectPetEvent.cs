...
using Retro.Communication.Packets.Outgoing.Pets;
    ...
    class RespectPetEvent : IPacketEvent
    {
        public void Parse(Hotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (!Session.GetHabbo().CurrentRoom.GetRoomUserManager().TryGetPet(PetId, out Pet)) 
            {
                ...
                {
                    Session.SendWhisper("Wow, you can not use that on yourself! (You have not missed a point, just recharge!)");
                    return;
                }

                //And boom! Let us send some respect points.
                RetroEnvironment.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.SOCIAL_RESPECT);
                RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_RespectGiven", 1);
                RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(TargetUser.GetClient(), "ACH_RespectEarned", 1);
            ...
            Session.GetHabbo().GetStats().DailyPetRespectPoints -= 1;
            RetroEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_PetRespectGiver", 1, false);
