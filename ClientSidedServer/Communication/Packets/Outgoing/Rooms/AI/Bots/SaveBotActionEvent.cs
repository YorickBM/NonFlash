                ...
                case 2:
                    {
                        ...
                        for (int i = 0; i <= SpeechData.Length - 1; i++)
                        {
                            using (IQueryAdapter dbClient = RetroEnvironment.GetDatabaseManager().GetQueryReactor())
                            {
                                ...
                                dbClient.AddParameter("MixChat", RetroEnvironment.BoolToEnum(Convert.ToBoolean(MixChat)));
                                dbClient.RunQuery();
                            }
                        }
                        #endregion
                ...
                case 5:
                    {
                        ...
                        {
                            Session.SendWhisper("Come on, at least give the bot a name!");
                            return;
                        }
                        else if (DataString.Length >= 16)
                        {
                            Session.SendWhisper("Come on, the bot does not need such a long name!");
                            return;
                        }
