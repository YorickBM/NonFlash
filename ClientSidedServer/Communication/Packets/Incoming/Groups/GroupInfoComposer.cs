...
namespace Retro.Communication.Packets.Outgoing.Groups
{
	class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
			...
			WriteString((RetroEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "No room found ..." : RetroEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
			...
			WriteString(RetroEnvironment.GetUsernameById(Group.CreatorId));
